...
const [isSettingsModalVisible, setIsSettingsModalVisible] = useState(false);
 ...
 useEffect(() => {
  ...
  const renderCircularTimer = () => {
    ...
    // No need for calculating the circumference and strokeDashoffset since we are not drawing the circle anymore
    ...
    return (
        <Svg
            ...
            style={{ alignItems: 'center', justifyContent: 'center' }} // Center the SVG in its container
        >
            {/* Display the time text at the center */}
            <SvgText
                x="50%" // Use percentages to center the text within the SVG
                y="50%" // Use percentages to center the text within the SVG
                dy={radius * 0.1} // Adjust dy to vertically center the text more accurately
                textAnchor="middle"
                fontSize="50"
                fontWeight="bold"
                fill="white"
  ...
  return (
    <ImageBackground 
    source={require('../../assets/background/city.gif')} 
    ...
    <View style={styles.container}>
      <View style={styles.pickerContainer}>
        <Picker
          ...
          <Picker.Item label="Select a task" value="" color="#fff"/>
          {tasks.map(task => (
            <Picker.Item key={task.id} label={task.title} value={task.id} color="#fff"/>
          ))}
        </Picker>
      ...
      {/* Render the rest period message and image if it's a rest session */}
       {(sessionType === 'short-rest' || sessionType === 'long-rest') && (
        <View style={restPeriodStyles.container}>
          <Image
            source={require('../../assets/Icons/rest2.png')} // Make sure the path is correct
